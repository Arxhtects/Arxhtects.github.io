...
function buidPage(nftBlocks, nftMeta) {
  if(nftBlocks.length >= 1) {
    ...
    for(let i = 0; i < nftMeta.length; i++) {
      ...
      $("#data > .wrapper").append("<div class='metabox' data-subtype='nft'><div class='left-wrap'><div class='type nft'><span>SubType</span><h4>NFT Meta</h4></div><div class='card'><h4><span>Name</span><br />" + nftMeta[i]['name'] + "</h4></div></div><div class='info-wrap'><h4><span>description</span><br />" + nftMeta[i]['description'] + "</h4><h4><span>Image</span><br />" + nftMeta[i]['image'] +"</h4><h4><span>Issuer</span><br />" + nftMeta[i].properties['issuer'] + "</h4><h4><span>Supply Block Hash</span><br />" + nftMeta[i].properties['supply_block_hash'] +"</h4></div><div class='end'><h4 class='view-jsonraw' onClick='Showme(\'nft" + i + "\')'><span>View</span><br />Raw Json</h4></div><div class='json-wrap' id='jsonnft" + i + "'><div>" + JSON.stringify(nftMeta[i]) + "</div></div></div>");
    }
    var seen = {};
